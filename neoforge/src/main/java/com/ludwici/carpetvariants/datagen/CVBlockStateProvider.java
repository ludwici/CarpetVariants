...
public class CVBlockStateProvider extends BlockStateProvider {
    ...
    private void createVariantCarpet(Block variant, Block orig) {
        ModelFile model = new ConfiguredModel(models().singleTexture(name(variant), fromNamespaceAndPath(CarpetVariantsMod.MODID, "block/carpet_north"), "wool", blockTexture(orig))).model;
